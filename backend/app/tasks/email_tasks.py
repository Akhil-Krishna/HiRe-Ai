import logging

from app.core.celery_app import celery_app
from app.core.config import settings
from app.services.email_service import (
    send_interview_invite_sync,
    send_interviewer_notification_sync,
)

logger = logging.getLogger(__name__)


@celery_app.task(
    bind=True,
    autoretry_for=(Exception,),
    retry_backoff=settings.CELERY_TASK_RETRY_BACKOFF,
    retry_jitter=settings.CELERY_TASK_RETRY_JITTER,
    retry_kwargs={"max_retries": settings.CELERY_TASK_MAX_RETRIES},
    name="app.tasks.email.send_interview_invite",
)
def send_interview_invite_task(self, payload: dict) -> bool:
    try:
        return bool(send_interview_invite_sync(**payload))
    except Exception as exc:
        logger.exception("Interview invite task failed: %s", exc)
        raise


@celery_app.task(
    bind=True,
    autoretry_for=(Exception,),
    retry_backoff=settings.CELERY_TASK_RETRY_BACKOFF,
    retry_jitter=settings.CELERY_TASK_RETRY_JITTER,
    retry_kwargs={"max_retries": settings.CELERY_TASK_MAX_RETRIES},
    name="app.tasks.email.send_interviewer_notification",
)
def send_interviewer_notification_task(self, payload: dict) -> bool:
    try:
        return bool(send_interviewer_notification_sync(**payload))
    except Exception as exc:
        logger.exception("Interviewer notification task failed: %s", exc)
        raise

